...
def gen_index_json():
    ...
    for pkg in svd_archives:
        ...
        print(' gzip', end='')
        pkg_file_gzip = 'data/' + pkg + '.tar.gz'
        compress_archive_gz(pkg_file_gzip, pkg_files)
        pkg_size_gzip = os.stat(pkg_file_gzip).st_size
        pkg_hash_gzip = get_file_hash(pkg_file_gzip)
        pkg_info['files']['gzip'] = {
            'name': pkg_file_gzip,
            'size': pkg_size_gzip,
            'hash': pkg_hash_gzip,
        ...
        if have_zstd:
            ...
            pkg_file_zstd = 'data/' + pkg + '.tar.zstd'
            compress_archive_zstd(pkg_file_zstd, pkg_files)
            pkg_size_zstd = os.stat(pkg_file_zstd).st_size
            pkg_hash_zstd = get_file_hash(pkg_file_zstd)
            pkg_info['files']['zstd'] = {
                'name': pkg_file_zstd,
                'size': pkg_size_zstd,
                'hash': pkg_hash_zstd,
    ...
    print(' hash', end='')
    with open(os.path.join(os.path.dirname(__file__), 'index.hash'), 'w') as f:
